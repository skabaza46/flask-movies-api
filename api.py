...
app.config["SECRET_KEY"] = "310999c0-4690-455c-9e5c-061957778915" # fake key for development***
...
class MovieSchema(ma.SQLAlchemyAutoSchema, PaginationSchema):
    ...
    class Meta:
        model = Movie
        # Fields to expose
        fields = ("title", "type", "date_added", "release_year", 'rating', 'duration', 'description', 'directors', 'casts','listings','countrys')
...
def search(searchVar=None):
    if not searchVar:
        ...
        query = Movie.query.filter(or_(Movie.title.contains(searchVar),
